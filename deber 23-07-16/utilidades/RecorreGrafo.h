...
#ifndef _RECORREGRAFO2_H
#define _RECORREGRAFO2_H
...
RecorreGrafo::RecorreGrafo()
{					// constructor
	CLAVE = 0xffff; // Clave vértice no marcado
}

int *RecorreGrafo::recorrerAnchuraMa(GrafoMatriz g, int verticeInicial)
{ // recorre grafo en anchura recibiendo numero de vertice inicial
	Cola cola;
	int *distancias;

	if (verticeInicial < 0 || verticeInicial > g.getNumVerts())
	{
		return distancias; // Vértice no existe
	}
	distancias = new int[g.getNumVerts()];
	...
	distancias[verticeInicial] = 0; // vértice origen queda marcado
	...
	while (!cola.isEmpty())
	{
		...
		cout << "Vertice [" << verticeActual << "] => " << g.getVertice(verticeActual).getDato() << " => visitado" << endl;

		for (int i = 0; i < g.getNumVerts(); i++)
		{
			if (g.getArco(verticeActual, i) && distancias[i] == CLAVE)
			{ // se marca vértice u con número de arcos hasta el
				distancias[i] = distancias[verticeActual] + 1;
				cola.enqueue(i);
			}
		}
	}
...
int *RecorreGrafo::recorrerAnchuraMa(GrafoMatriz g, TipoG org)
{ // recorre grafo en anchura recibiendo nombre de vertice inicial
...
int *RecorreGrafo::recorrerProfMa(GrafoMatriz g, int v)
{ // recorre grafo en profundidad recibiendo numero de vertice inicial
	int w;
	Pila pila;
	int *m;
	...
	while (!pila.isEmpty())
	{
		...
		{
			if (g.getArco(w, u) != 0xffff && m[u] == CLAVE)
			{ // se marca vértice u con número de arcos hasta el
				m[u] = m[w] + 1;
				pila.push(u);
			}
...
int *RecorreGrafo::recorrerProfMa(GrafoMatriz g, TipoG org)
{ // recorre grafo en profundidad recibiendo nombre de vertice inicial
	int v = g.getNumVertice(org);
	if (v >= 0)
	{
		recorrerProfMa(g, v);
...
int *RecorreGrafo::recorrerAnchuraLa(GrafoLista g, int v)
{ // recorre grafo en anchura recibiendo numero de vertice inicial
	int w;
	Cola cola;
	int *m;
	...
	// inicializa los vértices como no marcados
	for (int i = 0; i < g.getNumVerts(); i++)
	{ // inicializa las claves de cada vertice
		m[i] = CLAVE;
	}
	m[v] = 0; // vértice origen queda marcado
	cola.enqueue(v);
...
int *RecorreGrafo::recorrerAnchuraLa(GrafoLista g, TipoG org)
{ // recorre grafo en anchura recibiendo nombre de vertice inicial
...
int *RecorreGrafo::recorrerProfLa(GrafoLista g, int v)
{ // recorre grafo en profundidad recibiendo numero de vertice inicial
	int w;
	Pila pila;
	int *m;
