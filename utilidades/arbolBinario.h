...
NodoBinario* ArbolBinario::insertar(NodoBinario* rSub, Tipo valor) {
	if(rSub == NULL) { // arbol vacio
		rSub = new NodoBinario(valor);
		return rSub;
	}

	if(valor == rSub->getDato()) { // si valor se repite no ingreses
		return rSub;
	}
...
NodoBinario* ArbolBinario::buscar(NodoBinario* rSub, Tipo v) {
	if(rSub == NULL) { // Se recorrio toda la rama y no se encontro
		return NULL;
	}

	if(v == rSub->getDato()) { // Se encontro
		return rSub;
	}
