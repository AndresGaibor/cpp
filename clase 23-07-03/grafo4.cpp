// Grafo3.cpp
// Lista ordenada de los vertices que forman un grafo, en funcion de su grado de entrada
...
{
	cout << endl
		 << "Vertice #" << x.getNum() + 1 << " - " << x.getDato() << " - ";
}

void mostrarLosVerticesOrdenadosPorGrado(GrafoLista g, int asc = 1)
{
	ListaG ordenado;
	int cantVertices = g.cantidadVertices();
	int gradosEntrada[cantVertices];

	for (int i = 0; i < cantVertices; i++)
	{
		ListaG arcos = g.listaAdyacencia(i);

		NodoG *j = arcos.getPrimero();

		while (j != nullptr)
		{
			int va = g.getNumVertice(j->getDato());
	...
	for (int i = 0; i < cantVertices; i++)
	{
		tituloVertice(g.getVertice(i));

		cout << "Grado " << gradosEntrada[i];
	}
}
